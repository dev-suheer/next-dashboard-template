...
There is no backend, no API routes, and no data fetching; `middleware.ts` is the only server code. Every rendered value comes from `mock-data/dashboard.ts` at module scope. The only outbound request is the DiceBear avatar image in `projects-table.tsx`. If real data is ever introduced, wire it in properly rather than bolting it onto the module-scope imports.
